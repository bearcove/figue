...
All configuration goes through `figue::builder()`. The only standalone function
is `figue::from_slice()` for backwards compatibility (CLI-only parsing):

```rust
// Backwards compat: CLI args only, no env/file support
let args: Args = figue::from_slice(&["--verbose", "file.txt"])?;
