...
fn test_bool_chain_simple() {
    ...
    // Test `-abc` → `-a -b -c`
    let args: Args = figue::from_slice(&["-abc"]).unwrap();
    assert!(args.flag_a);
    assert!(args.flag_b);
    assert!(args.flag_c);
}
...
fn test_bool_repeated() {
    ...
    let args: Args = figue::from_slice(&["-vvv"]).unwrap();
...
fn test_bool_chain_with_value() {
    ...
    // Test `-abj4` → `-a -b -j 4`
    let args: Args = figue::from_slice(&["-abj4"]).unwrap();
    assert!(args.flag_a);
    assert!(args.flag_b);
...
fn test_backward_compat_attached_value() {
    ...
    let args: Args = figue::from_slice(&["-j4", "file.txt"]).unwrap();
    assert_eq!(args.jobs, 4);
...
fn test_bool_chain_with_separated_value() {
    ...
    let args: Args = figue::from_slice(&["-ab", "-j", "4"]).unwrap();
    assert!(args.flag_a);
    assert!(args.flag_b);
    assert_eq!(args.jobs, 4);
}
...
fn test_mixed_long_and_chained() {
    ...
    // Test `--verbose -ab`
    let args: Args = figue::from_slice(&["--verbose", "-ab"]).unwrap();
    assert!(args.verbose);
    assert!(args.flag_a);
    assert!(args.flag_b);
}
...
fn test_chain_with_equals() {
    ...
    let args: Args = figue::from_slice(&["-a=true"]).unwrap();
    assert!(args.flag_a);
}
...
fn test_chain_in_variant() {
    ...
    let args: Args = figue::from_slice(&["build", "-rv"]).unwrap();
    match args.command {
...
fn test_single_char_chain() {
    ...
    let args: Args = figue::from_slice(&["-v"]).unwrap();
    assert!(args.verbose);
}
...
fn test_chain_implicit_short() {
    ...
    let args: Args = figue::from_slice(&["-ab"]).unwrap();
    assert!(args.alpha);
    assert!(args.beta);
}
...
fn test_long_chain() {
    ...
    // Test `-abcde`
    let args: Args = figue::from_slice(&["-abcde"]).unwrap();
    assert!(args.flag_a);
    assert!(args.flag_b);
    assert!(args.flag_c);
    assert!(args.flag_d);
...
fn test_chain_with_positional() {
    ...
    // Test `-ab file.txt`
    let args: Args = figue::from_slice(&["-ab", "file.txt"]).unwrap();
    assert!(args.flag_a);
    assert!(args.flag_b);
    assert_eq!(args.path, "file.txt");
}
